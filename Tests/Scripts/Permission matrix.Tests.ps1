...
Describe 'Permission Matrix Tests' {
    BeforeDiscovery {
        # TestDrive not available
        
        # Helper path resolved safely
        $helpersPath = Join-Path (Split-Path -Parent $MyInvocation.MyCommand.Path) 'Helpers/Helpers.HC.ps1'
        . $helpersPath
    ...
    Describe 'create an error log file when' {
        ...
        It 'fails when <Property> path is invalid' -TestCases $InvalidPathTests {
            ...
            $testNewInputFile = Copy-ObjectHC $testInputFile
            $testNewInputFile.$Property = $Value
            ...
            (Test-GetLogFileDataHC)[0].Message | 
            Should -BeLike "*$Property '$Value' not found*"
        } -Tag test
