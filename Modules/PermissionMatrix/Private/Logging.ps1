...
function Get-DiagnosticsFieldReferenceHC {
    ...
    return [ordered]@{
        ...
        HowToRead   = [ordered]@{
            'AclProjectMsPerItem up and AceUnresolvedSids above zero' = 'Orphaned security identifiers. Entries left behind by deleted accounts cannot be resolved to a name and are not cached like successful lookups, so each one can cost a domain controller round trip on every item that carries it. Find them with the per-path AceUnresolvedSids column, clean them out of the ACLs, and the projection cost falls without any code change.'
            'AclProjectMsPerItem up and IdentityDistinct large'  = 'The tree references too many different accounts and groups for the machine to keep cached. Cost here is driven by variety, not volume, so it will not show up in ItemsWalked or AceCountMean.'
        ...
        Caveats     = @(
            'The identity census runs on 1 item in 1024, far more sparsely than the timers, because it is the only counter that walks the entry list and so the only one whose cost scales with entries rather than items. IdentityDistinct and AceUnresolvedSids are therefore lower bounds on what the tree actually contains. They answer "roughly how many" and "does this happen at all", not "exactly which".'
        ...
        PathFields   = [ordered]@{
            ...
            IdentityDistinct = [ordered]@{ Unit = 'count'; Meaning = 'Distinct identity names sampled in this subtree only. A single subtree well above the others is where the wide ACLs live.' }
        ...
        TelemetryFields = [ordered]@{
            ...
            IdentityDistinct       = [ordered]@{ Unit = 'count'; Meaning = 'How many DISTINCT account and group names the sampled ACLs referenced. This is the number that drives translation cost: Windows caches each security identifier it resolves, so a handful of groups repeated across millions of items is nearly free while thousands of them miss the cache constantly. AceCountMean cannot see this - twenty entries naming the same twenty groups everywhere and twenty naming a different twenty per folder look identical there. Sampled far more sparsely than the timers (1 item in 1024), so treat it as an order of magnitude, not an inventory.' }
            IdentityObservations   = [ordered]@{ Unit = 'count'; Meaning = 'How many individual access-control entries the identity census looked at. The denominator for AceUnresolvedPct, and the sample size behind IdentityDistinct.' }
            IdentityTruncated      = [ordered]@{ Unit = 'true/false'; Meaning = 'True when at least one walker job hit the 512-name cap and stopped collecting. IdentityDistinct is then a floor, not a count - read it as "at least this many".' }
            AceUnresolvedSids      = [ordered]@{ Unit = 'count'; Meaning = 'Sampled entries whose security identifier Windows could NOT resolve to an account name, usually left behind by a deleted account or group. Detected by type: .NET leaves an unresolvable IdentityReference as a SecurityIdentifier rather than an NTAccount. These are the expensive ones - a failed lookup is not cached the way a successful one is, so each occurrence can mean another round trip to a domain controller.' }
            AceUnresolvedItems     = [ordered]@{ Unit = 'count'; Meaning = 'Sampled items carrying at least one unresolvable entry. Compare with AceUnresolvedSids: a high ratio of entries to items means the orphans are concentrated on a few items, a low ratio means they are spread across the tree and inherited from somewhere near the top.' }
            AceUnresolvedPct       = [ordered]@{ Unit = 'percent'; Meaning = 'AceUnresolvedSids as a percentage of IdentityObservations. Anything above zero is worth cleaning up; a few percent across a large tree is enough to explain a projection cost that rose with no code or matrix change.' }
